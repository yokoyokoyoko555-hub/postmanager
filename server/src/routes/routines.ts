...
function toRoutineData(data: z.infer<typeof routineSchema>) {
  return { ...data, endDate: data.endDate ? new Date(data.endDate) : null };
}
...
router.post("/", ah(async (req, res) => {
  ...
  const routine = await prisma.routinePost.create({ data: toRoutineData(parsed.data) });
...
router.put("/:id", ah(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() });
  try {
    const routine = await prisma.routinePost.update({ where: { id: req.params.id }, data: toRoutineData(parsed.data) });
    res.json(routine);
  } catch (e) {
